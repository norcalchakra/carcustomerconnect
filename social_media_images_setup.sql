-- Create a new storage bucket for social media images
...
-- Add a function to track image usage
CREATE OR REPLACE FUNCTION public.track_image_usage()
RETURNS TRIGGER AS $$
BEGIN
  -- Insert a record into image_usage table (we'll create this next)
...
-- Create image_usage table to track which images are being used
...
-- Add trigger to track image usage when social posts are created
