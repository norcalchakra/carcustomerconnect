-- Comprehensive fix for Supabase storage issues (SAFE VERSION)
-- This script addresses all permission and configuration issues for the social-media-images bucket
-- This version checks if policies exist before creating them to avoid errors
...
BEGIN
  ...
  IF EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE policyname = 'Allow authenticated users to view images' 
    AND tablename = 'objects' 
    AND schemaname = 'storage'
  ) THEN
    DROP POLICY "Allow authenticated users to view images" ON storage.objects;
  END IF;
END $$;

-- 3. Create new policies with proper permissions, only if they don't already exist

-- Allow authenticated users to upload images (more permissive policy)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
...
-- Add a temporary permissive policy for direct API uploads
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE policyname = 'Allow direct API uploads to social media images' 
    AND tablename = 'objects' 
    AND schemaname = 'storage'
  ) THEN
    CREATE POLICY "Allow direct API uploads to social media images"
    ON storage.objects
    FOR INSERT
    TO public
    WITH CHECK (bucket_id = 'social-media-images');
