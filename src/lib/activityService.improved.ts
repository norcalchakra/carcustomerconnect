...
import { SocialPost } from './socialPostsApi.improved';
...
export const fetchRecentActivity = async (dealershipId: number, limit: number = 5): Promise<RecentActivity[]> => {
  try {
    ...
    return events.map((event: any) => ({
      ...
      notes: truncateText(event.notes, 60)
    }));
  } catch (error) {
...
export const fetchSocialMediaActivity = async (dealershipId: number, limit: number = 3): Promise<RecentActivity[]> => {
  try {
    ...
    return socialPosts.map((post: any) => {
      ...
      // Use content_summary if available, otherwise truncate content
      const displayContent = socialPost.content_summary || truncateText(socialPost.content, 60);
      
      // Get first image URL if available
      const imageUrl = socialPost.image_urls && socialPost.image_urls.length > 0 
        ? socialPost.image_urls[0] 
        : undefined;
      ...
      return {
        ...
        notes: displayContent,
        isSocialPost: true,
        platforms: socialPost.platform ? [socialPost.platform] : [],
        postId: socialPost.post_id,
        postUrl: socialPost.post_url,
        imageUrl
      };
    });
  } catch (error) {
